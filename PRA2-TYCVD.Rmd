...
En cuanto a los 2 registros con un espacio en blanco en la variable Embarked, como Southampton (S) es donde embarca más gente, se decide atribuir estos dos registros a ese puerto.
...
filas=dim(dataset)[1]
ggplot(data=dataset[1:filas,],aes(x=Sex,fill=Survived))+geom_bar()
````

````{r echo=TRUE, message=FALSE, warning=FALSE}

# Otro punto de vista. Survival como función de Embarked:
ggplot(data = dataset[1:filas,],aes(x=Embarked,fill=Survived))+geom_bar(position="fill")+ylab("Frecuencia")
````
...
````{r echo=TRUE, message=FALSE, warning=FALSE}
t<-table(dataset[1:filas,]$Embarked,dataset[1:filas,]$Survived)
for (i in 1:dim(t)[1]){
    t[i,]<-t[i,]/sum(t[i,])*100
}
t
