...
````{r echo=TRUE, message=FALSE, warning=FALSE}
supervivientes <- dataset$Age[dataset$Survived == 0] # Datos de supervivientes
noSupervivientes <- dataset$Age[dataset$Survived == 1] # Datos de los no supervivientes
````
...
* **Hipótesis alternativa:** el promedio de las edades de los supervivientes es mayor a el promedio de las edadesd de los no supervivientes

$$H_0: µ_1 > µ_2$$

Antes de aplicar el estadístico del contraste realizaremos el test de homocesdasticidad  para comprobar si las varianzas son iguales:
...
Como podemos observar los resultados coinciden y se obtiene un p-valor inferior al alfa definido de 0.05 por lo que podemos asumir que las crianzas son diferentes. 

Al estar ante unas varianzas diferentes el estadistico a a plicar será el contraste de dos muestras sobre la media con viaranzas diferentes.
...
* **Hipótesis nula:** la proporción de hombres que sobrevivieron es igual a la proporción de mujeres quesobrevivieron

$$H_0: p_1 = p_2$$


* **Hipótesis alternativa:** la proporción de hombres que sobrevivieron es diferente a la proproción de mujeres que sobrevivieron 
...
````{r echo=TRUE, message=FALSE, warning=FALSE}
alfa <- 1-0.95
x1 <- man[man == 1] # Hombres supervivientes
x2 <- woman[woman == 1] # Mujeres supervivientes

n1 <- length(man) # número de hombres en el barco
n2 <- length(woman) # número de mujeres en el barco 

p1 <- sum(length(x1))/n1 # proproción de hombres supervivientes
p2 <- sum(length(x2))/n2 # proporción de muejres supervivientes
...
Obtenemos que en ámbos casos el p-valor es igual por lo que podemos aceptar la hipótesis nula debido a que el p-valor obtenido es superior al alfa de 0.05 por lo que concluimos que la proporción de hombres supervivientes es igual a la proporción de mujeres supervivientes con un nivel de confianza del 95%.
...
````{r echo=TRUE, message=FALSE, warning=FALSE}
predicted_positive <- sum(d_test$Survived==1)
predicted_negative <- sum(d_test$Survived==0)
total <- predicted_positive + predicted_negative
````

````{r echo=TRUE, message=FALSE, warning=FALSE}
print(predicted_positive/total*100)
````

````{r echo=TRUE, message=FALSE, warning=FALSE}

print(predicted_negative/total*100)
````
